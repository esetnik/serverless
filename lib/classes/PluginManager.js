...
const requireServicePlugin = (pluginPath, localPluginsPath) => {
  this.serverless.cli.log(
    `loading plugin at path ${pluginPath}, localPluginsPath ${localPluginsPath}`
  );
  ...
  resolveServicePlugins(servicePlugs) {
    ...
    return pluginsObject.modules
      ...
      .map(name => {
        let Plugin;
        try {
          Plugin = requireServicePlugin(name, pluginsObject.localPath);
        } catch (error) {
